...
public:
  ...
  CraneArm(std::string name) :
    as_(nh_, name, false),
    action_name_(name)
  {
    as_.registerGoalCallback(boost::bind(&CraneArm::goalCB, this));

	sub_joint_state = nh_.subscribe<sensor_msgs::JointState>("/joint_states", 1, &CraneArm::JointStateCallback, this);
	
    pub_move_arm_ = nh_.advertise<trajectory_msgs::JointTrajectory>("/joint_trajectory", 1, this);
    as_.start();
  }
...
bool CraneArm::isWithinRange(std::map<std::string, double> lhs,
							std::map<std::string, double> rhs,
							double full_range)
{
  bool ret = true;
...
{
  ros::init(argc, argv, "crane_plus_arm_controller/follow_joint_trajectory");

  CraneArm Cranearm(ros::this_node::getName());
  ros::spin();
